...
Shader "Hidden/CustomWaterShadow" {
Properties {
...
UNITY_DECLARE_DEPTH_TEXTURE(_WaterDepthMap);
...
*/
inline float3 computeCameraSpacePosFromDepthAndInvProjMat(v2f i, out float4 waterPos)
{
    float zdepth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv.xy);
	float zdepthWater = SAMPLE_DEPTH_TEXTURE(_WaterDepthMap, i.uv.xy);

    #if defined(UNITY_REVERSED_Z)
        zdepth = 1 - zdepth;
		zdepthWater = 1 - zdepthWater;
    #endif
	...
	float4 clipPos = float4(i.uv.zw, zdepthWater, 1.0);
    clipPos.xyz = 2.0f * clipPos.xyz - 1.0f;
    float4 camPos = mul(unity_CameraInvProjection, clipPos);
    camPos.xyz /= camPos.w;
    camPos.z *= -1;

	waterPos.xyz = camPos.xyz;
	waterPos.w = zdepth > zdepthWater;

    clipPos = float4(i.uv.zw, zdepth, 1.0);
    clipPos.xyz = 2.0f * clipPos.xyz - 1.0f;
    camPos = mul(unity_CameraInvProjection, clipPos);
    camPos.xyz /= camPos.w;
    camPos.z *= -1;
...
*/
inline float3 computeCameraSpacePosFromDepthAndVSInfo(v2f i, out float4 waterPos)
{
    float zdepth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv.xy);
	float zdepthWater = SAMPLE_DEPTH_TEXTURE(_WaterDepthMap, i.uv.xy);
	...
	float depthWater = lerp(Linear01Depth(zdepthWater), zdepthWater, unity_OrthoParams.w);
#if defined(UNITY_REVERSED_Z)
    zdepth = 1 - zdepth;
	zdepthWater = 1 - zdepthWater;
#endif

	float3 vposPersp = i.ray * depthWater;
    float3 vposOrtho = lerp(i.orthoPosNear, i.orthoPosFar, zdepthWater);
    ...
    vposPersp = i.ray * depth;
    vposOrtho = lerp(i.orthoPosNear, i.orthoPosFar, zdepth);
    // pick the perspective or ortho position as needed
    camPos = lerp(vposPersp, vposOrtho, unity_OrthoParams.w);
    return camPos.xyz;
}

inline float3 computeCameraSpacePosFromDepth(v2f i, out float4 waterPos);
...
fixed4 frag_hard (v2f i) : SV_Target
{
	...
	float4 vposWat;
    float3 vpos = computeCameraSpacePosFromDepth(i, vposWat);

    float4 wpos = mul (unity_CameraToWorld, float4(vpos,1));
	float4 wposWat = mul (unity_CameraToWorld, float4(vposWat.xyz,1));

    fixed4 cascadeWeights = GET_CASCADE_WEIGHTS (wpos, vpos.z);
    float4 shadowCoord = GET_SHADOW_COORDINATES(wpos, cascadeWeights);

	fixed4 cascadeWeightsWat = GET_CASCADE_WEIGHTS (wposWat, vposWat.z);
    float4 shadowCoordWat = GET_SHADOW_COORDINATES(wposWat, cascadeWeightsWat);

    //1 tap hard shadow
    fixed shadow = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord);
    shadow = lerp(_LightShadowData.r, 1.0, shadow);

	fixed shadowWat = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoordWat);
    shadowWat = lerp(_LightShadowData.r, 1.0, shadowWat);
...
half GetShadowSoft(float4 wpos, float3 vpos)
{
	fixed4 cascadeWeights = GET_CASCADE_WEIGHTS(wpos, vpos.z);
...
fixed4 frag_pcfSoft(v2f i) : SV_Target
{
	...
	float4 vposWater; 
    float3 vpos = computeCameraSpacePosFromDepth(i, vposWater);

    // sample the cascade the pixel belongs to
    float4 wpos = mul(unity_CameraToWorld, float4(vpos,1));
	float4 wposWater = mul(unity_CameraToWorld, float4(vposWater.xyz,1));

	fixed4 shadow = fixed4(GetShadowSoft(wpos, vpos), 0, 0, 0);
	shadow.b = GetShadowSoft(wposWater, vposWater.xyz).r;
...
SubShader {
    ...
    Pass {
        ...
        inline float3 computeCameraSpacePosFromDepth(v2f i, out float4 camPos)
        {
            return computeCameraSpacePosFromDepthAndVSInfo(i, camPos);
        }
        ENDCG
    }
}
...
SubShader {
    ...
    Pass{
        ...
        inline float3 computeCameraSpacePosFromDepth(v2f i, out float4 camPos)
        {
            return computeCameraSpacePosFromDepthAndInvProjMat(i, camPos);
        }
        ENDCG
    }
}
