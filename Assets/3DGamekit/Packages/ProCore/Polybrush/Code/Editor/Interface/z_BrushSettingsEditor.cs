...
namespace Polybrush
{
	...
	public class z_BrushSettingsEditor : Editor
	{
		...
		public void OnEnable()
		{
			...
			gc_Radius = new GUIContent(z_IconUtility.GetIcon("Icon/Radius"), "Radius: The distance from the center of a brush to it's outer edge.\n\nShortcut: 'Ctrl + Mouse Wheel'");
			gc_Falloff = new GUIContent(z_IconUtility.GetIcon("Icon/InnerRadius"), "Inner Radius: The distance from the center of a brush at which the strength begins to linearly taper to 0.  This value is normalized, 1 means the entire brush gets full strength, 0 means the very center point of a brush is full strength and the edges are 0.\n\nShortcut: 'Shift + Mouse Wheel'");
			gc_FalloffCurve = new GUIContent(z_IconUtility.GetIcon("Icon/Falloff"), "Falloff: Sets the Falloff Curve.");
			gc_Strength = new GUIContent(z_IconUtility.GetIcon("Icon/Strength"), "Strength: The effectiveness of this brush.  The actual applied strength also depends on the Falloff setting.\n\nShortcut: 'Ctrl + Shift + Mouse Wheel'");
