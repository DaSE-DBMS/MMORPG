...
#elif defined(_COLORSCREEN_ON)
#define fragColorMode(i) \
    albedo.rgb = 1 - (1 - albedo.rgb) * (1 - i.color.rgb); \
    albedo.a *= i.color.a;
#elif defined(_COLORDIFFERENCE_ON)
#define fragColorMode(i) \
    albedo.rgb = abs(i.color.rgb - albedo.rgb); \
    albedo.a *= i.color.a;
#elif defined(_COLORADDITIVE_ON)
#define fragColorMode(i) \
    albedo.rgb += i.color.rgb; \
    albedo.a *= i.color.a;
#elif defined(_COLORSUBTRACTIVE_ON)
#define fragColorMode(i) \
    albedo.rgb -= i.color.rgb; \
    albedo.a *= i.color.a;
#else
...
half4 fragParticleUnlit (VertexOutput IN) : SV_Target
{
    ...
    #if defined(_ALPHAMODULATE_ON)
    result.rgb = lerp(1.0, albedo.rgb, albedo.a);
    #endif
